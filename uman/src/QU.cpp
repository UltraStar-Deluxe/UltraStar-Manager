...
bool QU::fileTypeLessThan(const QFileInfo &fi1, const QFileInfo &fi2) {
	...
	if( (QUSongSupport::allowedSongFiles() << QUSongSupport::allowedKaraokeFiles() ).contains(suffix1)
		and (QUSongSupport::allowedSongFiles() << QUSongSupport::allowedKaraokeFiles() ).contains(suffix2)) {
		foreach(QString type, QUSongSupport::allowedSongFiles() << QUSongSupport::allowedKaraokeFiles()) {
